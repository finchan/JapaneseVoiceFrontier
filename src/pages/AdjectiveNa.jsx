...
export default function AdjectiveNa() {
    const [query, setQuery] = useState('');
    const [adjNaInfo, setAdjNaInfo] = useState(null);
            ...
            <div className="vc-row flex flex-col md:flex-row md:flex-wrap items-start md:items-center gap-3">
                ...
                {adjNaInfo && (
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl border w-full md:w-auto md:ml-auto"
                        ...
                        {adjNaInfo.meaning && (
                            <span className="text-xs" style={{ color: colors.textLight }}>{adjNaInfo.meaning}</span>
                        )}
                    </div>
                )}
                {searchStatus === 'notfound' && (
                    <span className="text-xs font-bold" style={{ color: colors.error }}>な形容词が見つかりません</span>
                )}
                {searchStatus === 'error' && (
